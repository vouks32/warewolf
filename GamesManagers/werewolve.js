...
export class WereWolvesManager {
    ...
    async addPrayer(playerJid, whatsapp) {
        ...
        if (!playerJid || !whatsapp) return false
        console.log(`Adding prayer to ${playerJid}`, whatsapp?.ids)
    ...
    async usePrayer(playerJid, whatsapp) {

        if (!playerJid || !whatsapp) return false
        console.log(`Using prayer for ${playerJid}`, whatsapp?.ids)
        let user = getUser(playerJid)
    ...
    async chooseGameType(groupId, whatsapp) {
        ...
        this.games[groupId] = {
            ...
            gameType: null, // "POINTS" ou "francs"
    ...
    async createGame(groupId, whatsapp) {
        ...
        if (this.games[groupId].gameType === 1) {
            ...
            if (hostUser && hostUser.francs >= 5) {
                await SaveUsersfrancs(this.games[groupId].hostjid, whatsapp, -5, "a lancé une partie de loup avec mise en jeu", "WEREWOLVE", 1, this.games[groupId])
    ...
    async startGame(groupId, whatsapp) {
        ...
        for (let i = 0; i < game.players.length; i++) {
            ...
            if (p.role === "WEREWOLF" && !game.alphaWerewolfInGame) {
                ...
                if ((Math.random() < 0.8 && game.players.length < 10 && game.players.length > 7) || (Math.random() < 0.7 && game.players.length > 9)) {
    ...
    async resolveNight(groupId, whatsapp) {
            ...
            for (const victimId in counts) {
                ...
                if (game.prostituteProtected && game.prostituteProtected.includes(victimId)) {
                    ...
                    if (victim.role === "HUNTER") {
                        ...
                        if (victimUser) {
                            victimUser.lastDeathNight = game.nights
                            saveUser(victimUser)
                        }
                        await whatsapp.sendMessage(groupId, `@${victimId.split('@')[0]} a été tué pendant la nuit! il était [${victim.role}]`, [victimId])
        ...
        if (result) {
            ...
            const names = game.players
                .sort((p, q) => (p.points.reduce((sum, v) => sum + v.points, 0) > q.points.reduce((sum, v) => sum + v.points, 0) ? 1 : -1))
                .sort((p, q) => (winners.some(w => w.jid === q.jid) ? 1 : -1))
                .map((p, i) => (winners.some(w => w.jid === p.jid) ? '🏆' : '💩') +
                    ` *${p.name}* (@${p.jid.split('@')[0]}) ` +
                    (!p.isDead ? `😀` : `☠️`) +
                    ' [' + p.role + "]\n- *(" +
                    (p.points.reduce((sum, v) => sum + v.points, 0) >= 0 ? '+' : '') + p.points.reduce((sum, v) => sum + v.points, 0) + " points)*" +
                    (game.gameType == 2 && p.points.reduce((s, v) => s + v.points, 0) >= 0 && winners.some(w => w.jid === p.jid) ? "- (" + Math.round(totalpoints > 0 && p.points.reduce((sum, v) => sum + v.points, 0) > 0 ? ((p.points.reduce((sum, v) => sum + v.points, 0) / totalpoints) * paidMise) : 0) + " francs)" : "")).join("\n\n")
            ...
            if (game.gameType == 2)
                for (const p of game.players) {
                    ...
                    if (p.points.reduce((sum, v) => sum + v.points, 0) >= 0 && winners.some(w => w.jid === p.jid)) {
    ...
    async resolveVotes(groupId, whatsapp) {
        ...
        if (TANNERWASVOTED) {
            ...
            SaveUsersfrancs(game.players.find(p => p.role === "TANNER").jid, whatsapp, paidMise, "End werewolf francs TANNER", "WEREWOLVE", 0,)
            this.addUserPoints(game.players.find(p => p.role === "TANNER").jid, whatsapp, pointsList(game).votedAsTanner, 'voté en tant que tanner', 0)

            const names = game.players.
                sort((p, q) => (q.role === "TANNER" ? 1 : -1)).
                map((p, i) => (p.role === "TANNER" ? '🏆' : '💩') + ` *${p.name}* (@${p.jid.split('@')[0]}) ` +
                    (!p.isDead ? `😀` : `☠️`) + ' [' + p.role + "]\n- *(" + (p.points.reduce((sum, v) => sum + v.points, 0) >= 0 ? '+' : '') + p.points.reduce((sum, v) => sum + v.points, 0) + " points)" + (game.gameType === 2 ? "*\n- " + (p.role === "TANNER" ? paidMise : 0) + " francs" : "")).join("\n\n")
            const mentions = game.players.map((p, i) => p.jid)
            await whatsapp.sendMessage(groupId, "Joueurs :\n\n " + names, mentions)
            await whatsapp.sendMessage(groupId, `envoie *"!werewolve"* pour rejouer`)
        ...
        if (result) {
            ...
            await whatsapp.sendMessage(groupId, `🏆 Partie terminée! \n*${result}* gagnent!\nLes gagnants recoivent *+${winpoints} points*\nLes Perdants recoivent *${-losepoints} points*`)
            this
            const totalpoints = game.players.filter(p => winners.some(w => w.jid === p.jid)).filter(p => p.points.reduce((s, v) => s + v.points, 0) >= 0).reduce((sum, p) => sum + p.points.reduce((s, v) => s + v.points, 0), 0)
            const paidMise = game.mise * (95 / 100)
            ...
            const names = game.players
                ...
                .map((p, i) => (winners.some(w => w.jid === p.jid) ? '🏆' : '💩') +
                    ...
                    ' [' + p.role + "]\n- *(" +
                    (p.points.reduce((sum, v) => sum + v.points, 0) >= 0 ? '+' : '') + p.points.reduce((sum, v) => sum + v.points, 0) + " points)*" +
                    (game.gameType == 2 && p.points.reduce((s, v) => s + v.points, 0) >= 0 && winners.some(w => w.jid === p.jid) && totalpoints > 0 ? "- (" + Math.round(totalpoints > 0 && p.points.reduce((sum, v) => sum + v.points, 0) > 0 ? ((p.points.reduce((sum, v) => sum + v.points, 0) / totalpoints) * paidMise) : 0) + " francs)" : "")).join("\n\n")
            const mentions = game.players.map((p, i) => p.jid)
            await whatsapp.sendMessage(groupId, "Joueurs :\n\n" + names, mentions)
            await whatsapp.sendMessage(groupId, `envoie *"!werewolve"* pour rejouer`)
    ...
    async wolfKill(groupId, wolfJid, targetJid, whatsapp) {
        ...
        await whatsapp.sendMessage(groupId, ([`🐺 Les loups-garous hurlent à la pleine lune.`, `🐺 Dés hurlements retentissent dans tout le village`, `🐺 Des hurlements de loups se mélangent à ceux de leurs victimes`])[Math.floor(Math.random() * 3)])
    ...
    async wolfTransform(groupId, wolfJid, targetJid, whatsapp) {
        ...
        await whatsapp.sendMessage(target.jid, `🐺 Tu as été transformé en loup!`)
    }
    ...
    async Pray(groupId, whatsapp) {
        const game = this.games[groupId]
        if (!game || game.state !== "NIGHT") return await whatsapp.sendMessage(whatsapp.sender, "⚠️ Tu ne peux prier qu'une fois la nuit tombée.")

        const prayingPlayer = game.players.find(p => p.jid === whatsapp.sender)
        const prayingUser = await getUser(whatsapp.sender)
        if (!prayingPlayer || prayingPlayer.isDead) {
            await whatsapp.sendMessage(whatsapp.sender, "⚠️ Tu ne peux pas utiliser la capacité de Prière, tu es mort.")
            return
        }
